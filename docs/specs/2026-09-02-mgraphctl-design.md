...
class GraphClient:
    ...
    def paginate(self, path: str, *, params=None, beta=None, outlook_tz=False, limit: int | None,
                 all_: bool, cap: int, page_size: int | None,
...
`path` is already URL-encoded (built with `odata.p()`), may be absolute (`https://graph.microsoft.com/...`, used for `@odata.nextLink`) or relative (`/me/messages`); relative paths are prefixed with `https://graph.microsoft.com/v1.0` or `/beta` (`beta=True` per call, or the global `--beta`). `token_provider(force_refresh)` comes from `auth`. `PageResult = (items: list[dict], truncated: bool, pages: int, fetched: int | None)` where `fetched` (read through `PageResult.fetched_count`) is how many items the fetch returned before any client-side post-filter; `http.filter_page(page, keep)` is the one way to apply such a filter and preserves `truncated`/`pages`/`fetched` from the fetch; `http.with_routing(page, **ids)` stamps the ids Graph omits when a message is fetched through its own collection (`chatId` on chat messages, `teamId`/`channelId` on channel messages and replies), never overwriting one Graph did supply; `DownloadResult = (path, bytes, content_type)`; `BatchRequest = (id, method, url, headers, body)`; `BatchResponse = (id, status, headers, body, error: GraphError | None)` (`error` is set for any sub-status ≥ 400); `paginate`'s `stop` ends the fetch at the first item it accepts, for a feed the server already orders the way the caller reads it. The items it stopped at and beyond are still returned, for the caller's own `filter_page` to judge, so once `stop` has fired only the items *ahead* of it were ever asked for: the page is `truncated` when there are more of those than the bound, and not otherwise — never because of the tail past the end or the unfollowed `@odata.nextLink` that leads further past it. `SearchResult = (hits: list[dict], total: int | None, more: bool)` where `hits` are the flattened `hitsContainers[].hits[]` entries (each with `hitId`, `rank`, `summary`, `resource`).
...
| Mode | Rule |
|---|---|
| JSON (`--json`) | exactly one pretty-printed (`indent=2`, `ensure_ascii=False`, sorted keys off) document on stdout. Lists: `{"items":[...],"count":N,"fetched":N,"cap":N|null,"truncated":bool}`, plus `"window":{"after","before"}` (ISO strings or null) on every command that accepts a time window — present even when both bounds are unset, absent entirely on commands that accept none — and `"query"` with the server-side query actually sent. `fetched` is what Graph returned before any client-side pass and is always emitted (equal to `count` when nothing was filtered, so a consumer never branches on the key); `cap` is the bound the fetch ran under (`--limit`, or the noun's cap under `--all`). `cap`, `fetched` and `truncated` describe the fetch and are never recomputed from the filtered list. `ListResult.page` carries the `PageResult` so all three come from one place; `window`/`query` are typed fields threaded from the `graph/*` layer, not re-derived in `render.py`. Single objects: the Graph object as returned (fields restricted by `$select`, never renamed). Writes: the created/updated Graph object, or `{"status":"sent"}` / `{"status":"deleted","id":...}` / `{"status":"accepted"}` when Graph returns no body; multi-id writes (`mail mark ID...`) use the list envelope. Dry run: `{"dryRun":true,"requests":[{"method","url","headers","body"}]}`. Errors never go to stdout. |
...
| `update` | `ID` | any `create` option except `--calendar` (Graph cannot move an event between calendars), all optional | `PATCH /me/events/{id}` with only the given fields; tz | `Calendars.ReadWrite` | — | P1 | W; updating a series master updates the series (documented) |
...
| `channel messages` | `TEAM CHANNEL` | `--limit 20`, `--all` (cap 200), `--full`, `--with-replies`, `--replies MSGID`, `--after DT`, `--before DT` | `GET /teams/{t}/channels/{c}/messages?$top=50[&$expand=replies]`; `--replies`: `GET …/messages/{m}/replies?$top=50` (paged, `--limit` honoured — fixes quirk 8); `messageType=unknownFutureValue` and deleted messages dropped | `ChannelMessage.Read.All` | 5.7 messages / replies | P0 | text chronological (oldest first) with Markdown bodies, 300 chars unless `--full`; JSON = Graph order (newest first). `--after`/`--before` add **no** query parameter: Graph documents `$top` and `$expand` as this endpoint's only supported ones (v1.0 and beta), so a `$filter` would be rejected or silently ignored. The window is applied client-side on the reply chain's last-modified time (`graph.teams.chain_modified`), which is the order Graph returns; being newest-first, paging stops at the first message older than `--after` (`paginate(stop=…)`) and stopping there is not a truncation. Cap stays 200, as for chat messages. `--after`/`--before` with `--replies`, or `--after` later than `--before`, is exit 2 |
...
| Verb | Args | Options (default) | Graph call | Scopes | Parity | Tier | Notes |
|---|---|---|---|---|---|---|---|
| `list` | — | `--unread`, `--type oneOnOne\|group\|meeting`, `--since DT`, `--limit 20`, `--all` (cap 200) | `GET /me/chats?$top=50&$expand=members,lastMessagePreview&$orderby=lastMessagePreview/createdDateTime desc&$select=id,topic,chatType,lastUpdatedDateTime,viewpoint,webUrl`; unread = `viewpoint.lastMessageReadDateTime < lastMessagePreview.createdDateTime` (client-side, adds `_unread` in text only); `--since` adds no parameter — the feed is already in that order, so paging stops at the first chat whose `lastMessagePreview.createdDateTime` predates it (`paginate(stop=…)`) and the rest are dropped by `filter_page` | `Chat.Read` | 5.6 chats | P0 | topic for 1:1 = other member's displayName; `--since` adds a `lastMessage` column in text mode |
| `get` | `CHAT` | — | `GET /chats/{id}?$expand=members` | `Chat.Read` | — | P1 | |
| `members` | `CHAT` | — | `GET /chats/{id}/members` | `Chat.Read` | — | P1 | |
| `messages` | `CHAT` | `--limit 20`, `--all` (cap 200), `--full`, `--after DT`, `--before DT` | `GET /chats/{id}/messages?$top=50&$orderby=createdDateTime desc`; windowed: `$orderby=lastModifiedDateTime desc&$filter=lastModifiedDateTime gt {after} and lastModifiedDateTime lt {before}` | `Chat.Read` | 5.6 messages | P0 | text chronological, Markdown bodies, `[image: hostedContents/<id>]` markers; JSON Graph order (documented in SKILL). The window uses `lastModifiedDateTime` because Graph supports `gt`/`lt` only there (`createdDateTime` takes `lt` alone) **and** ignores a `$filter` whose property `$orderby` does not name — so the bounds mean *last touched*. `--after` later than `--before` is exit 2 |
...
| `search` | `Q` | `--after DT`, `--before DT`, `--limit 25`, `--all` (cap 200) | `POST /search/query {requests:[{entityTypes:["chatMessage"], query:{queryString}, from, size:25}]}` paging on `moreResultsAvailable`; date filter client-side on `createdDateTime` | `Chat.Read` (chats), `ChannelMessage.Read.All` (channel hits) | 5.6 search | P0 | each hit rendered with `chat:<id>` or `channel:<teamId>/<channelId>` in the `where` text column (fixes quirk 16), and the same routing as structured `kind` (`chat`\|`channel`\|`unknown`), `chatId`, `teamId`, `channelId` so a JSON consumer follows a hit into a windowed fetch without parsing `where` apart; a hit with no routing, or half a `channelIdentity`, is `unknown` with an empty `where`; body = `summary` snippet (documented) |
...
| `transcript` | `[MEETING] TRANSCRIPT_ID` | `--join-url` \| `--event`, `--format text\|vtt` (text), `--speakers`, `--output FILE` | `GET /me/onlineMeetings/{m}/transcripts/{t}/content?$format=text/vtt` (stream); `text` = local `vtt_to_text` (`[HH:MM:SS] Speaker: line`, one per cue), `--speakers` = local `vtt_to_speaker_turns` (`**Speaker:** text`, consecutive cues from one speaker merged, blank line between turns); 403 `SpeakerAttributionNotAllowed` → retry with `Accept: application/vnd.microsoft.graph.transcript+text`; with `--json` one extra `GET /me/onlineMeetings/{m}/transcripts` for `createdDateTime` | `OnlineMeetingTranscript.Read.All` | 5.14 --transcript | P0 | JSON `{"meetingId","transcriptId","createdDateTime","format","content"}`. `format` is sniffed from the body (`meetings.looks_like_vtt`), never taken from `--format` — the attribution fallback answers in text to a vtt request — and `FileResult.meta` reports the same sniffed value. `createdDateTime` is best-effort: `null` when the lookup fails, still exit 0, and only the `--json` path pays for it. `--speakers` with `--format vtt` is exit 2 |
